...
• Built a company based on Innovation and User-Centric Design
...
def display_welcome_interface():
    ...
    # Gemini badge
    st.markdown(
        '<div class="gemini-badge">🤖 Enhanced with Google Gemini AI</div>',
        unsafe_allow_html=True
    )
...
def display_sidebar(ai_brain, db_manager):
    ...
    with st.sidebar:
        st.markdown("---")
        st.subheader("🏢 WellSoft Corporation")
...
def main():
    ...
    if not os.environ.get("GEMINI_API_KEY"):
        st.error("⚠️ GEMINI_API_KEY not found in environment variables. Please add it to use AI features.")
